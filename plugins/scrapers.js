...
Asena.addCommand({pattern: 'audio ?(.*)', fromMe: false, desc: Lang.SONG_DESC}, (async (message, match) => { 
    ...
    got.stream(arama[0].image).pipe(fs.createWriteStream(__dirname+'/files/'+title + '.jpg'));
    ffmpeg(stream)
        .audioBitrate(320)
        .save(__dirname+'/files/' + title + '.mp3')
        .on('end', async () => {
            const writer = new ID3Writer(fs.readFileSync(__dirname+'/files/' + title + '.mp3'));
            writer.setFrame('TIT2', arama[0].title)
                .setFrame('TPE1', [arama[0].author.name])
                .setFrame('APIC', {
                    type: 3,
                    data: fs.readFileSync(__dirname+'/files/'+title + '.jpg'),
...
Asena.addCommand({pattern: 'video ?(.*)', fromMe: false, desc: Lang.VIDEO_DESC}, (async (message, match) => { 
    ...
    yt.pipe(fs.createWriteStream(__dirname+'/files/' + arama.videoId + '.mp4'));

    yt.on('end', async () => {
        reply = await message.client.sendMessage(message.jid,Lang.UPLOADING_VIDEO,MessageType.text);
        await message.client.sendMessage(message.jid,fs.readFileSync(__dirname+'/files/' + arama.videoId + '.mp4'), MessageType.video, {mimetype: Mimetype.mp4});
    });
}));

Asena.addCommand({pattern: 'buscar ?(.*)', fromMe: false, desc: Lang.YT_DESC}, (async (message, match) => { 
